...
홈 히어로 영상은 3.5MB다. 이 필드는 JS 한 장(32.6KB, gzip 10.3KB)으로 같은 계열의 인상을 내고,
해상도에 걸리지 않으며, 팔레트를 런타임에 바꿀 수 있다. 대신 프레임 예산을
직접 관리해야 한다 — 아래 참조.

## 프리셋

| 이름 | 용도 | 프리셋 기본 intensity |
|---|---|---|
| `gold` | 레퍼런스 재현. 골드 / 근접 블랙 | 1.0 |
| `brand` | 같은 필드를 브랜드 레드로 | 0.62 |

**서브페이지 히어로는 프리셋 기본값을 안 쓴다.** `hero-field.js`가
`intensity: 0.58`, `coreScale: 0.45`, `maxDpr: 1.5`, `loopSeconds: 18`로 덮어쓴다.
밝기를 조정할 곳은 프리셋이 아니라 거기다.
...
| `coreScale` | 크레스트만 따로 누름. 대비에는 거의 영향이 없다 — 위 참조 |
...
`prefers-reduced-motion: reduce`여도 **캔버스는 만들고 정지 프레임 한 장을 그린다**
(`start()`가 `frame(0.18)`만 부르고 루프를 안 돈다). 움직임만 빼는 것이지 그림까지
빼는 게 아니다 — 아래에 깔린 게 없어서, 안 그리면 빈 히어로가 된다.
캔버스는 장식이라 `.hero-field`에 `aria-hidden="true"`가 붙어 있다.

**대비는 반드시 루프 전체에서 재야 한다.** 움직이는 배경이므로 한 프레임만 재면
최악을 놓친다. 루프를 8등분해 각 지점에서 글리프 줄마다 가장 밝은 배경 픽셀을 찾고
그중 최악을 취한다. `intensity: 0.58`은 그 측정으로 나온 값이다.

`coreScale`은 아니다. 크레스트가 글리프에 얹히는 게 원인이라고 보고 넣었지만,
0.35~0.55로 훑어보니 최악값이 0.01도 안 움직였다 — 한계 픽셀은 크레스트가 아니라
반짝이 덩어리다. 지금 0.45인 건 순전히 보기 때문이고, 대비 문제를 이걸로 고치려
들면 안 된다.
...
> 잘못 치환돼 무한 재귀에 빠졌는데, 서브페이지는 `intensity`를 명시로 넘기므로 `??`가
