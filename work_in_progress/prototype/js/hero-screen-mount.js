...
import { HeroScreen } from './hero-screen.js?v=14';
...
if (canvas && media) {
  /* Subpages sit at 0.58 behind a copy column. The wall can take a little more, but four
     additive crests already supply the extra light — going back to 1 brought the red mass
     back. */
  const screen = new HeroScreen(canvas, media, {
    intensity: 0.64, maxDpr: 1.5,
  ...
  const begin = () => { screen.resize(); screen.frame(screen.still ? 0.18 : 0); sync(); };
